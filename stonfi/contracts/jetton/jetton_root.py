...
class JettonRoot:
    ...
    @staticmethod
    def create_from_address(address: Union[Address, str]) -> Type["JettonRoot"]:
        return JettonRoot(address)

    async def get_wallet_address(self, owner_address: Union[Address, str], provider: LiteClientLike) -> Address:
    ...
    async def get_wallet(self, owner_address: Union[Address, str], provider: LiteClientLike) -> JettonWallet:
        return JettonWallet.create_from_address(await self.get_wallet_address(owner_address, provider))
    
    async def get_jetton_data(self, provider: LiteClientLike) -> list:
        stack = await provider.run_get_method(address=self.address,
